...
# Connecting to NetGuard Certificate Manager / Insta Certifier
...
- modify the est proxy configuration (`est_proxy.cfg`) and add the following parameters
...
- handler_file - path and name of ca handler
...
Depending on CA policy configuration a CSR may require approval. In such a situation est_proxy will poll the CA server to check the CSR status. The polling interval can be configured in est_proxy.cfg.
